...
import { getBranding, getSharedNote } from '@/lib/repository';
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  if (!note) return { title: `Shared note · ${branding.appName}`, robots: { index: false, follow: false } };
  return { title: `${note.title || 'Shared note'} · ${branding.appName}`, description: note.content.slice(0, 150), robots: { index: false, follow: false } };
}
